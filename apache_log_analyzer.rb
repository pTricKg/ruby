...
class ApacheLogAnalyzer
  ...
  def analyze(file_name)
    ...
    File.open(file_name).each do |line|
      # a line of code to find the IP address
      ip = ip_regex.match(line).to_s 
      # a line of code to find the URL
      url = url_regex.match(line).to_s
      # does the URL contain the word "secret"?
      if url.include?("secret")
        secret = true
      end
      # does the line have a 404 error?
      if line.include?("404")
        error = true
      end
